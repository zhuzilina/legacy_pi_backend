...
class ChoiceQuestion(BaseQuestion):
    ...
    options = models.JSONField(default=list, verbose_name='选项', help_text='选择题的选项列表，格式：[{"text": "选项内容", "is_correct": true}]')
...
class FillQuestion(BaseQuestion):
    """填空题模型"""
    correct_answer = models.TextField(verbose_name='正确答案', help_text='填空题的正确答案，多个答案用分号分隔')
...
class Answer(models.Model):
    ...
    answer_type = models.CharField(max_length=10, choices=ANSWER_TYPE_CHOICES, default='choice', verbose_name='题目类型')
    choice_question = models.ForeignKey(ChoiceQuestion, on_delete=models.CASCADE, null=True, blank=True, related_name='answers', verbose_name='选择题')
    fill_question = models.ForeignKey(FillQuestion, on_delete=models.CASCADE, null=True, blank=True, related_name='answers', verbose_name='填空题')
    ...
    def __str__(self):
        ...
        if self.choice_question:
            question_text = self.choice_question.question_text[:30]
        elif self.fill_question:
            question_text = self.fill_question.question_text[:30]
    ...
    def get_question(self):
        ...
        if self.choice_question:
            ...
            return self.fill_question
        return None
    
    def clean(self):
        """验证数据"""
        from django.core.exceptions import ValidationError
        if not self.choice_question and not self.fill_question:
            raise ValidationError('必须关联一个题目')
        if self.choice_question and self.fill_question:
            raise ValidationError('不能同时关联选择题和填空题')
        if self.choice_question and self.answer_type != 'choice':
            raise ValidationError('选择题答案的answer_type必须为choice')
        if self.fill_question and self.answer_type != 'fill':
            raise ValidationError('填空题答案的answer_type必须为fill')
...
class DailyQuestion(models.Model):
    ...
    question_type = models.CharField(max_length=10, choices=[('choice', '选择题'), ('fill', '填空题')], verbose_name='题目类型')
    ...
    def __str__(self):
        return f"{self.client_ip} - {self.date} - {self.get_question_type_display()}"
