...
# Legacy PI Backend HTTP 部署脚本
# 使用方法: ./deploy_http.sh [操作]
# 示例: ./deploy_http.sh deploy
...
check_environment() {
    ...
    if [ ! -f "env.production" ]; then
        log_warning "生产环境配置文件 env.production 不存在，使用默认配置"
    fi
...
# 主部署函数
deploy() {
    log_info "开始部署 Legacy PI Backend 到生产环境 (HTTP 模式)..."
    ...
    log_success "部署完成！"
    log_info "服务访问地址: http://your-domain.com"
    log_info "管理界面: http://your-domain.com/admin/"
    log_info "API 文档: http://your-domain.com/api/"
}
...
main() {
    case "${1:-deploy}" in
        ...
        "health")
            ...
            ;;
        *)
            echo "使用方法: $0 [deploy|stop|restart|logs|backup|health]"
            echo ""
            echo "命令说明:"
            echo "  deploy   - 完整部署到生产环境 (HTTP 模式)"
            echo "  stop     - 停止所有服务"
            echo "  restart  - 重启所有服务"
            echo "  logs     - 显示服务日志"
            echo "  backup   - 备份数据"
