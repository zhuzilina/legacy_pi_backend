...
# Docker 镜像导出脚本
# 用于在本地导出所有需要的镜像，以便在无法访问 Docker 仓库的云端环境中使用
...
export_images() {
    ...
    docker save redis:7.2-alpine | gzip > docker-images/redis-7.2-alpine.tar.gz
    
    # 导出 MongoDB 镜像
    log_info "导出 MongoDB 7.0..."
    docker save mongo:7.0 | gzip > docker-images/mongo-7.0.tar.gz
    
    # 导出 Django 应用镜像
    log_info "导出 Django 应用镜像..."
    docker save legacy_pi_backend_django-app:latest | gzip > docker-images/legacy_pi_backend_django-app-latest.tar.gz
...
generate_import_script() {
    ...
    cat > docker-images/import-images.sh << 'EOF'
#!/bin/bash
...
# 检查镜像文件是否存在
if [ ! -d "." ]; then
    echo "错误: 当前目录不是 docker-images 目录"
    exit 1
fi
...
echo "导入 Django 应用镜像..."
gunzip -c legacy_pi_backend_django-app-latest.tar.gz | docker load
...
EOF
    ...
    chmod +x docker-images/import-images.sh
...
generate_deployment_guide() {
    ...
    cat > docker-images/DEPLOYMENT_README.md << 'EOF'
# 云端部署说明
...
- `legacy_pi_backend_django-app-latest.tar.gz` - Django 应用镜像
- `import-images.sh` - 镜像导入脚本

## 部署步骤

1. 将整个 `docker-images` 目录上传到云端服务器
2. 进入 `docker-images` 目录
3. 执行导入脚本: `./import-images.sh`
4. 返回项目根目录
5. 配置环境变量: `cp env.example env.production && nano env.production`
6. 执行部署: `./deploy_http.sh deploy`
...
- 确保网络端口 80 已开放
- 导入完成后可以删除 `docker-images` 目录以节省空间
EOF
...
show_export_results() {
    ...
    log_success "镜像导出完成！"
    log_info "下一步:"
    echo "1. 将 docker-images 目录上传到云端服务器"
    echo "2. 在云端服务器执行: cd docker-images && ./import-images.sh"
    echo "3. 返回项目根目录执行: ./deploy_http.sh deploy"
}
